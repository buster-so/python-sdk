...
def test_airflow_resource_initialization():
    ...
    assert client.airflow.v3.config == {}
    ...
    assert client_with_config.airflow.v3.config == config

def test_airflow_report_error(capsys, monkeypatch):
    """
    Verifies that report_error accepts arguments and calls send_request with expected data.
    """
    client = Client(buster_api_key="test-key")
...
def test_airflow_report_error_with_api_version(monkeypatch):
    ...
    client = Client(buster_api_key="test-key", airflow_config={"api_version": ApiVersion.V2})
...
def test_airflow_report_error_with_env(monkeypatch):
    ...
    # Pass env in config
    client = Client(buster_api_key="test-key", airflow_config={"env": Environment.STAGING})
